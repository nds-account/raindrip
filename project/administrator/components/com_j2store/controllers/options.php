...
class J2StoreControllerOptions extends J2StoreController {
    ...
    function save() {
		...
		if ($model->store($post)) {
			...
			$msg = $model->getError();
			if($post['option_id']) {
				$link = 'index.php?option=com_j2store&view=options&task=edit&cid[]='.$post['option_id'];
			} else {
				$link = 'index.php?option=com_j2store&view=options&task=add';
			}
		}
	...
	function setoptionvalues(){
		...
		if(!$option_id) {
			$errors[] = JText::_('J2STORE_OPTIONVALUES_OPTION_ID_MISSING');
		...
		if(!in_array($table->type, $type_array)) {
			$errors[] = JText::_('J2STORE_OPTIONVALUES_OPTION_TYPE_INVALID');
		}
		...
		$view->set( '_action', "index.php?option=com_j2store&view=options&task=setoptionvalues&id=".$model->getId()."&tmpl=component" );
	...
	function saveoptionvalues()
	{
		...
		$redirect = "index.php?option=com_j2store&view=options&task=setoptionvalues&id={".$option_id ."}&tmpl=component";
	...
	function createoptionvalue()
	{
		...
		if(!$option_id || !empty($optionvalue_name)) {
			...
			if(empty($optionvalue_name)) {
				$this->message = JText::_('J2STORE_OPTIONVALUES_OPTION_NAME_MISSING');
			} else {
				$this->message = JText::_('J2STORE_OPTIONVALUES_OPTION_ID_MISSING');
			}
		}
	...
	function deleteoptionvalue(){
		...
		if (!isset($this->redirect)) {
			...
			: 'index.php?option=com_j2store&view=options&task=setoptionvalues&id='.$option_id.'&tmpl=component';
