...
class J2StoreModelOrders extends J2StoreModel
{
	...
	function _buildQuery()
	{
		...
		$query->select('a.*')->from('#__j2store_orders AS a');
		//users table
		$query->select(' u.name AS buyer, u.email as bemail')
			 ->leftJoin('#__users AS u ON u.id = a.user_id');

		//orderinfo
		$query->select('oi.user_email, oi.user_id as oiuserid, oi.billing_first_name, oi.all_billing, oi.all_shipping, oi.all_payment')
