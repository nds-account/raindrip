...
class Com_J2StoreInstallerScript {
	...
	private function _doDBChanges($type='') {
		...
		if(!in_array($prefix.'j2store_productquantities', $tables)){
			...
			`manage_stock` smallint(5) NOT NULL DEFAULT '0',
			`min_out_qty` decimal(12,4) NOT NULL,
			`use_store_config_min_out_qty` smallint(5) NOT NULL DEFAULT '1',
			`min_sale_qty` decimal(5,0) NOT NULL,
			`use_store_config_min_sale_qty` smallint(5) NOT NULL DEFAULT '1',
			`max_sale_qty` decimal(12,4) NOT NULL,
			`use_store_config_max_sale_qty` smallint(5) NOT NULL DEFAULT '1',
			`notify_qty` decimal(12,4) NOT NULL,
			`use_store_config_notify_qty` smallint(5) NOT NULL DEFAULT '1',
	...
	private function _modifyExistingTables($type='') {
		...
		if(in_array($prefix.'j2store_productquantities', $tables)){
			$fields = $db->getTableColumns('#__j2store_productquantities');

			if (!array_key_exists('manage_stock', $fields)) {
				$query = "ALTER TABLE #__j2store_productquantities ADD `manage_stock` smallint(5) NOT NULL DEFAULT '0' AFTER `quantity`";
				$this->_executeQuery($query);
			}

			if (!array_key_exists('min_out_qty', $fields)) {
				$query = "ALTER TABLE #__j2store_productquantities ADD `min_out_qty` decimal(12,4) NOT NULL AFTER `manage_stock`";
				$this->_executeQuery($query);
			}

			if (!array_key_exists('use_store_config_min_out_qty', $fields)) {
				$query = "ALTER TABLE #__j2store_productquantities ADD `use_store_config_min_out_qty` smallint(5) NOT NULL DEFAULT '1' AFTER `min_out_qty`";
				$this->_executeQuery($query);
			}

			if (!array_key_exists('min_sale_qty', $fields)) {
				$query = "ALTER TABLE #__j2store_productquantities ADD `min_sale_qty` decimal(12,4) NOT NULL AFTER `use_store_config_min_out_qty`";
				$this->_executeQuery($query);
			}

			if (!array_key_exists('use_store_config_min_sale_qty', $fields)) {
				$query = "ALTER TABLE #__j2store_productquantities ADD `use_store_config_min_sale_qty` smallint(5) NOT NULL DEFAULT '1' AFTER `min_sale_qty`";
				$this->_executeQuery($query);
			}

			if (!array_key_exists('max_sale_qty', $fields)) {
				$query = "ALTER TABLE #__j2store_productquantities ADD `max_sale_qty` decimal(12,4) NOT NULL AFTER `use_store_config_min_sale_qty`";
				$this->_executeQuery($query);
			}

			if (!array_key_exists('use_store_config_max_sale_qty', $fields)) {
				$query = "ALTER TABLE #__j2store_productquantities ADD `use_store_config_max_sale_qty` smallint(5) NOT NULL DEFAULT '1' AFTER `max_sale_qty`";
				$this->_executeQuery($query);
			}

			if (!array_key_exists('notify_qty', $fields)) {
				$query = "ALTER TABLE #__j2store_productquantities ADD `notify_qty` decimal(12,4) NOT NULL AFTER `use_store_config_max_sale_qty`";
				$this->_executeQuery($query);
			}

			if (!array_key_exists('use_store_config_notify_qty', $fields)) {
				$query = "ALTER TABLE #__j2store_productquantities ADD `use_store_config_notify_qty` smallint(5) NOT NULL DEFAULT '1' AFTER `notify_qty`";
				$this->_executeQuery($query);
			}
			...
			if(count($rows) < 1) {
				...
				if(isset($currency_code) && strlen($currency_code)) {
					...
					if($row->store_id) {
						...
						if($currency_id) {
							$store->config_currency = $currency_id;
