...
class plgSystemModalsHelper
{
		...
		if (JFactory::getApplication()->input->getInt('ml', 0) && JFactory::getApplication()->input->getInt('iframe', 0))
		{
			...
			if (preg_match_all($regex, $str, $matches, PREG_SET_ORDER) > 0)
			{
				foreach ($matches as $match)
				{
					...
					if (empty($attribs->href) || $attribs->href['0'] != '#' || isset($attribs->target))
					{
						continue;
					}

					// ignore if link is external or an image
					if ($this->isExternal($attribs->href) || $this->isMedia($attribs->href))
					{
						continue;
					}
	...
	function buildLink($l, $data, $content = '')
	{
		...
		if ($ismedia)
		{
			...
			if ($auto_titles)
			{
				$data['title'] = $this->getTitle($l->href, $title_case);
			}
		}
	...
	function getTitle($url, $case)
	{
		...
		switch ($case)
		{
			...
			case 'titlecase_smart':
				$title = ucwords(strtolower($title));
				$lowercase_words = explode(',', ' ' . str_replace(',', ' , ', strtolower($this->params->lowercase_words)) . ' ');
