...
//			General overlay options
				...
				overlayOpacity:over_opacity,	// 1 is opaque, 0 is completely transparent (change the color in the CSS file)
...
//			Global media options
				html5: yjishtml5,			// HTML5 settings for YouTube and Vimeo, false = off, true = on
...
//			NonverBlaster
				playerpath: plugin_path+'plugins/content/bot_mb/bot_mb/js/NonverBlaster.swf',	// Path to NonverBlaster.swf
...
//				options.overlayOpacity = 0.001;	// Helps ameliorate the issues with CSS overlays in iOS, leaving a clickable background, but avoiding the visible issues
			...
			top = window.getScrollTop() + (window.getHeight()/2);
			left = window.getScrollLeft() + (window.getWidth()/2);
			var srcMarginLeft = ( isNaN(media.getStyle('margin-left').toInt()) ) ? 0 : media.getStyle('margin-left').toInt();
			margin = center.getStyle('padding-left').toInt()+srcMarginLeft+media.getStyle('padding-left').toInt();
			marginBottom = bottom.getStyle('margin-left').toInt()+bottom.getStyle('padding-left').toInt()+bottom.getStyle('margin-right').toInt()+bottom.getStyle('padding-right').toInt();
			setTopMargin = (options.initialHeight/2)- margin;
			setLeftMargin = (options.initialWidth/2)- margin;
...
/*  clickBlock code - remove the following three lines to enable right-clicking on links to images  */
			...
			//var savetitle = el.title;
			links.removeEvents("click").addEvent("click", function() {
				
				// YJ CHANGE
				var savetitle = this.getElement('.Mbox_titles').get('html');
				this.set('title',savetitle);
...
//	$$('#mb_').each(function(hide) { hide.set('display', 'none'); });
    ...
    if (mboxes.length > 0) {
        mboxes.each(function (el) {
            var get_rel_data = el.getElement('.Mbox_reldata');
