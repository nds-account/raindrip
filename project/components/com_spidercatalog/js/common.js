...
function vote(vote_value,prid,div_id,rated_text)
{  
	if(resQ)   
		{  
			resQ=0;
			
		paramslist='option=com_spidercatalog&view=starrating&product_id='+prid+'&vote_value='+vote_value+'&format=row';  
		  
		SCHR.sendRequest('index.php', div_id ,paramslist);	
...
else  
{
	if(resQ) 
		{
			resQ=0;
  	  		SCHR.sendRequest(document.getElementById('wd_captcha_img').src+'&checkcap=1&cap_code='+document.getElementById("review_capcode").value, 'caphid','');
			resNumberOfTry=0;
...
document.getElementById('wd_captcha_img').src=document.getElementById('wd_captcha_img').src+'&r='+Math.floor(Math.random()*100);
...
function cat_form_resettiko(form)
{

form.prod_name.value='';
form.tiko.value='1';
if(typeof(form.cat_id)!=='undefined') form.cat_id.value='0';
form.subcat_id.value='0';
form.submit();
}
function cat_form_selectsubcat(form)
{
...
function cat_form_allcateg(form)
{
...
function prod_namesubmit(form)
{

form.workses.value='1';
form.submit();
}
