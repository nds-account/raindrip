...
if(count($rows))
echo '<table cellpadding="0" cellspacing="0" id="productMainTable" style="width:'. ($params->get('count_of_product_in_the_row')*$params->get('new_cell_all_width')+$params->get('count_of_product_in_the_row')*20).'px"><tr>';
...
$link=$po.$row->id.'/'.$cat_id.'/'.$titlelink;
   ...
   echo '<td>';
   ?>
   <div id="productMainDiv" style="padding:10px;
 <?php if(($params->get('price') and $row->cost != 0 and $row->cost != '') or ($params->get('market_price') and $row->market_cost != 0 and $row->market_cost != '') ) { echo   'background-image:url('.JURI::root().'components/com_spidercatalog/images/tuxt.png);';} ?>
   background-repeat:no-repeat; 
    background-position: 100% 0;
	background-size: 150px 140px;
	<?php echo 'border-width:' . $params->get('border_width') . 'px;border-color:#' . $params->get('border_color') . ';border-style:' . $params->get('border_style') . ';' . (($params->get('text_size_small') != '') ? ('font-size:' . $params->get('text_size_small') . 'px;') : '') . (($params->get('text_color') != '') ? ('color:#' . $params->get('text_color') . ';') : '') .  ' width:' . $params->get('new_cell_all_width') . 'px; height:' . $params->get('new_cell_all_height') . 'px;">' ?>

<div style=" <?php if(($params->get('price') and $row->cost != 0 and $row->cost != '') or ($params->get('market_price') and $row->market_cost != 0 and $row->market_cost != '') ) { echo 'background-image:url('.JURI::root().'components/com_spidercatalog/images/tuxt.png);'; } ?>
background-position: 107% -2.4%;background-size: 150px 140px;
   background-repeat:no-repeat;background-color:#<?php echo $params->get('background_color') ;?>; ">


   <?php
echo '<div style="height:' . ($params->get('new_cell_all_height') + 10) . 'px;">';
?>
<?php if(($params->get('price') and $row->cost != 0 and $row->cost != '') or ($params->get('market_price') and $row->market_cost != 0 and $row->market_cost != '') ){
echo '<div id="prodTitle" style="padding:20px;height:50px;
background-image:url('.JURI::root().'components/com_spidercatalog/images/seriy.png);
background-size:'.(194-(320 - $params->get('new_cell_all_width') )).'px 100px;
background-repeat: no-repeat;color:#' . $params->get('cell_new_title_color').'">';
}
else
{
echo '<div id="prodTitle" style="padding:20px;height:50px;background-image:url('.JURI::root().'components/com_spidercatalog/images/seriy.png);background-size:125% 100px;
background-repeat: no-repeat;color:#' . $params->get('cell_new_title_color').'">';
}
?>
<?php
echo '<table><tr><td style="width:'.($params->get('new_cell_all_width')-145).'px;"><font size="6" style="font-size: ' . $params->get('cell_new_big_title_size') . 'pt;">' . $array[0] . '</font><span style="font-size:' . $params->get('cell_new_title_size') . 'pt"><br>'.$array2.'&nbsp;&nbsp;&nbsp;</span></td><td style="min-width: 100px;">';

if ($params->get('price') and $row->cost != 0 and $row->cost != '')
  echo '<div id="prodCost" style="text-align: right;font-size:' . $params->get('cell_new_price_size') . 'px;color:#d80303;">' . (($params->get('currency_symbol_position') == 0) ? ($params->get('currency_symbol')) : '') . ' ' . $row->cost . ' ' . (($params->get('currency_symbol_position') == 1) ? $params->get('currency_symbol') : '') . '</div>';
    
    
    if ($params->get('market_price') and $row->market_cost != 0 and $row->market_cost != '')
        echo '<div id="prodCost" style="text-align: right;font-size:' . ($params->get('cell_new_market_price_size')) . 'px;"><span style="color:#d80303;">' . JText::_('SH_MARKET_PRICE') . ' </span><span style=" text-decoration:line-through;color:#d80303;"><br> ' . (($params->get('currency_symbol_position') == 0) ? ($params->get('currency_symbol')) : '') . ' ' . $row->market_cost . ' ' . (($params->get('currency_symbol_position') == 1) ? $params->get('currency_symbol') : '') . '</span></div>';





echo '</tr></tr></table></div><table id="prodMiddle" border="0" cellspacing="0" cellpadding="0"><tr><td style="padding-left: 7px;">';
...
if ($params->get('enable_rating'))
        ...
        if ($voted[$id] == 0)
          {
            ...
            echo "<div id='voting" . $row->id . "' style='height:1px; padding:10px;'>
    ...
    if (!($row->image_url != "" and $row->image_url != ";"))
      {
        ...
        echo '<td style="vertical-align: middle !important;padding-right:15px;width:'.$params->get('cell_new_picture_width').'px;"><img  style="width:'.$params->get('cell_new_picture_width').'px;height:'.$params->get('cell_new_picture_height').'px;border:solid 1px #e5e5e5;" src="'.JURI::root().'/components/com_spidercatalog/images/noimage.jpg" />
	...
	if($params->get('cell_new_picture_height')=='')
				$paramheight=100;
	else
	
				$paramheight=$params->get('cell_new_picture_height');
				
	if($params->get('cell_new_picture_width')=='')
				$paramwidth=100;
	else
	
				$paramwidth=$params->get('cell_new_picture_width');
				
				
        echo '<td style="vertical-align: middle !important;padding-right:15px;width:'.$params->get('cell_new_picture_width').'px;"><a href="' . $imgurl[0] . '" target="_blank"><img style="width:' . $paramwidth . 'px; height:' . $paramheight . 'px;border:solid 1px #e5e5e5;" src="index.php?option=com_spidercatalog&view=picture&format=raw&tmpl=component&id=' . $row->id . '&picnum=0&width=170" /></a></td>';
		} else{
	if($params->get('cell_new_picture_width')>=$params->get('cell_new_picture_height'))
	
	 	  echo '<td style="vertical-align: middle !important;padding-right:15px;width:'.$params->get('cell_new_picture_width').'px;"><a href="' . $imgurl[0] . '" target="_blank"><img  style="width:'.$params->get('cell_new_picture_width').'px;border:solid 1px #e5e5e5;" src="index.php?option=com_spidercatalog&view=picture&format=raw&tmpl=component&id=' . $row->id . '&picnum=0&width='.$params->get('cell_new_picture_width').'" /></a></td>';
	 else{
	  echo '<td style="vertical-align: middle !important;padding-right:15px;width:'.$params->get('cell_new_picture_width').'px;"><a href="' . $imgurl[0] . '" target="_blank"><img style="height:'.$params->get('cell_new_picture_height').'px;border:solid 1px #e5e5e5;"  src="index.php?option=com_spidercatalog&view=picture&format=raw&tmpl=component&id=' . $row->id . '&picnum=0&height='.$params->get('cell_new_picture_height').'" /></a></td>';
	}
    }
	}
...
$small_images_str.='<a href="'.$img.'" target="_blank"><img  style="width:70px;height:50px;border:solid 1px #e5e5e5;margin:2px;" src="index.php?option=com_spidercatalog&view=picture&format=raw&tmpl=component&id='.$row->id.'&picnum='.$key.'&height=50" vspace="0" hspace="0" ></a>
';
$small_images_count++;
}
}
...
echo '</td>';
    ...
    echo "<table style='width:".$params->get('new_cell_all_width')."px;' border=\"0\" cellspacing=\"0\" cellpadding=\"0\" width=\"100%\">";
    
        $subcat_id=JRequest::getVar('subcat_id',$cat_id);
   // print_r($categories[1]);
   if ($row->category_id > 0 and $params->get('cell_show_category'))
	{
        echo '<tr style="font-size:'.$params->get('cell_new_text_size').'px;color:#'.$params->get('cell_new_text_color').';background-color:#'.$params->get('cell_new_text_back_color_1').'; vertical-align:middle;"><td style="padding-left:12px;padding-right:12px;padding-top:12px;"><b>' . JText::_('SH_CATEGORY') . '</b></td><td style="padding-left:12px;padding-right:12px;padding-top:12px;"><span id="cat_' . $row->id . '">'; 
...
echo '</span></td></tr>';
   ...
   else
   {
        ...
        if ($par_data[$j] != '')
          {
            ...
            $bgcolor = (($j % 2) ? (($params->get('cell_new_text_back_color_2') != '') ? ('background-color:#' . $params->get('cell_new_text_back_color_2') . ';') : '') : (($params->get('cell_new_text_back_color_1') != '') ? ('background-color:#' . $params->get('cell_new_text_back_color_1') . ';') : ''));
            
         
            if ($countOfPar != 0)
              {
                echo '<tr style="font-size:'.$params->get('cell_new_text_size').'px;' . $bgcolor . 'text-align:left"><td style="color:#'.$params->get('cell_new_text_color').';padding-left:12px;padding-right:12px;"><b>' . $par1_data[0] . ':</b></td>';
                

                    echo '<td style="padding-left:12px;color:#'.$params->get('cell_new_text_color').';padding-right:12px;width:' . $params->get('parameters_select_box_width') . 'px;"><ul class="spidercatalogparamslist">';
    ...
    echo '<div style="color:#'.$params->get('cell_new_text_color').';font-size:'.$params->get('cell_new_text_size').'px;padding-left:12px;padding-right:12px;background-color:#' . $params->get('cell_new_text_back_color_2') . ';"  id="prodDescription">' . $description[0] . '</div></td></tr>

</table><br>
	<a href="' . $link . '" style="position:absolute;bottom:20px;right:2%;display:block;color:#fff;text-align:center;text-decoration:none;width:89%;min-height:20px; background-color:#' . $params->get('cell_new_more_background_color') . ';font-size:' . $params->get('new_cell_more_font_size') . 'pt !important;padding:10px;">' . JText::_('SH_MORE_INFO') . '</a>
</div>
</div>
</div>';
