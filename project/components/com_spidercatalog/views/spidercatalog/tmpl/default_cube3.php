...
echo '</tr></tr></table></div>';
	...
	if($params->get('all_cell_picture_width')=='')
        ...
        echo '<td style="width:100%;"><a href="' . $imgurl[0] . '" target="_blank"><img  style="padding:10px;width:' . $paramwidth . 'px; height:' . $paramheight . 'px;" src="index.php?option=com_spidercatalog&view=picture&format=raw&tmpl=component&id=' . $row->id . '&picnum=0&width=' . $paramwidth . '&height=' . $paramheight . '" /></a></td>';
		} else{
	if($params->get('all_cell_picture_width')>=$params->get('all_cell_picture_height'))
	
	 echo '<td style="width:100%;"><center><a href="' . $imgurl[0] . '" target="_blank"><img style="height:'.$params->get('all_cell_picture_height').'px;" src="index.php?option=com_spidercatalog&view=picture&format=raw&tmpl=component&id=' . $row->id . '&picnum=0&width='.$params->get('all_cell_picture_height').'" /></a></center></td>';
	 else{
	 echo '<td style="width:100%;"><center><a href="' . $imgurl[0] . '" target="_blank"><img  style="height:'.$params->get('all_cell_picture_height').'px;" src="index.php?option=com_spidercatalog&view=picture&format=raw&tmpl=component&id=' . $row->id . '&picnum=0&height=' . $params->get('all_cell_picture_height'). '" /></a></center></td>';
	}
    }
	}
