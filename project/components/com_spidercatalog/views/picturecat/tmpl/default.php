...
$query="select category_image_url FROM #__spidercatalog_product_categories WHERE id=".JRequest::getVar('id') ;
$db->setQuery($query);
$row = $db->loadObject();
//print_r($row);
   //echo $row->image_url;
//print_r($row->image_url);
$imgurl=explode(";", $row->category_image_url);
$picnum=JRequest::getVar('picnum');

//echo $imgurl[0];

   
      $image_info = getimagesize($imgurl[$picnum]);
if ($image_info=='')
{
...
$doc->setMimeEncoding('image/jpeg');
        ...
        $this->image = imagecreatefromjpeg($imgurl[$picnum]);
...
$doc->setMimeEncoding('image/gif');
        ...
        $this->image = imagecreatefromgif($imgurl[$picnum]);
...
$doc->setMimeEncoding('image/png');
         ...
         $this->image = imagecreatefrompng($imgurl[$picnum]);
