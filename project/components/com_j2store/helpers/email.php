...
class J2StoreHelperEmail {
	...
	protected function processTags($text, $order, $extras=array()) {
		...
		//billing Format [CUSTOM_BILLING_LABEL:KEYNAME], [CUSTOM_BILLING_VALUE:KEYNAME]
		$text = $this->processCustomFields($order->order, 'billing', $text);

		//shipping Format [CUSTOM_SHIPPING_LABEL:KEYNAME], [CUSTOM_SHIPPING_VALUE:KEYNAME]
		$text = $this->processCustomFields($order->order, 'shipping', $text);

		//payment Format [CUSTOM_PAYMENT_LABEL:KEYNAME], [CUSTOM_PAYMENT_VALUE:KEYNAME]
